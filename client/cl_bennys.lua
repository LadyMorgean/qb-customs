...
function CheckRestrictions(location)
    ...
    -- Dynamic Benny's toggle (server-controlled via GlobalState)
    -- If bennys is disabled AND this location is considered a "bennys" shop, block entry.
    if Config.DynamicBennys and (GlobalState.qb_customs_bennysEnabled == false) then
        local treatAll = Config.TreatAllLocationsAsBennys == true
        local isBennysLocation = treatAll

        if not treatAll and Config.BennysLocations then
            isBennysLocation = Config.BennysLocations[location] == true
        end

        if isBennysLocation then
            return false
        end
    end
